...
fn main() {
    ...
    let mut middle = Cone::new(translation(0.0, 0.0, 0.0));
    middle.material.color = Color::new(1.0, 0.5, 0.2);
    let mut left = Cone::new(translation(5.0, 0.0, 0.0) * scaling(0.5, 0.5, 0.5) * rotation_y(PI / 3.0) * rotation_z(PI / 6.0));
    left.closed = true;
    left.minimum = -1.0;
    left.maximum = 1.0;
    left.material.color = Color::new(0.2, 0.5, 1.0);
    let mut right = Cone::new(translation(0.0, 0.0, 5.0) * scaling(0.65, 0.65, 0.65) * rotation_y(-2.0 * PI / 3.0) * rotation_z(PI / 6.0));
    right.closed = false;
    right.minimum = -1.0;
    right.maximum = 1.0;
